...
## Applied: surrogate ID + sequence on every table

18 of the 73 tables didn't follow the ID/IDENTITY pattern used everywhere
else — 14 lookup/master tables keyed on a business code (e.g. `STATUS_CODE`,
`PAY_MODE_CODE`) and 4 security junction tables keyed on a composite FK pair
(e.g. `SEC_USER_ROLES` on `USER_ID`+`ROLE_ID`). Fixed in `db/tables/` (and
regenerated into `scripts/deploy_full.sql`):

- Added a new `<name>_ID NUMBER GENERATED BY DEFAULT AS IDENTITY` column to
  each, now the table's `PRIMARY KEY`.
- The original key (code column, or composite pair) was **kept, not
  dropped** — demoted from `PRIMARY KEY` to a `UNIQUE` constraint.

This was deliberate: 9+ existing foreign keys reference these tables by
their original code column (e.g. `FLEET_REF_BIKE_STATUSES.STATUS_CODE`).
Oracle allows a foreign key to target any `UNIQUE` constraint, not only the
`PRIMARY KEY`, so keeping the code column `UNIQUE` means every existing FK
and every place in the packages/triggers that compares against the code
value keeps working unchanged — only the PK mechanism changed, not the
business key. Tables affected: `core_years`, `fleet_ref_bike_statuses`,
`fleet_ref_custody_types`, `fleet_ref_doc_types`, `fleet_sim_operators`,
`fleet_sim_status_master`, `hr_approval_modules`, `hr_doc_type_master`,
`hr_hiring_source_master`, `hr_org_doc_type_master`, `hr_pay_mode_master`,
`hr_pro_deal_incl_master`, `hr_pro_deal_type_master`, `hr_status_master`,
`sec_role_perms`, `sec_user_orgs`, `sec_user_projects`, `sec_user_roles`.
